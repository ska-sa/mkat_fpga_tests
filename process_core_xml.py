...
def process_latex_escapes(text):
    ...
    def replace(m):
        ...
        math_txt = math_txt.replace(r"\\", "\\").replace(r"\{", "{").replace(r"\}", "}")
        math_hash = "mathhashup" + hashlib.md5(math_txt).hexdigest()
        replacement_txt = "{}:math:`{}`{}".format(m.group("leading"), math_txt, m.group("trailing"))
        ...
        for attrib in data.get("attribute", []):
            if attrib.get("name"):
                if attrib.get("name") not in data:
                    ...
                    data["attrib-" + attrib.get("name")] = attrib.get("text")
        del (data["attribute"])
...
def core_xml_to_dict(node, depth=1, path=""):
    ...
    try:
        data = {"tag": _clean_xml_tag(node.tag), "path": path, "depth": depth}
        ...
        for child in node.getchildren():
            try:
                ...
                if client_tag not in data:
                    data[client_tag] = []
                data[client_tag].append(core_xml_to_dict(child, depth, path + "." + client_tag))
            except AttributeError as er:
                print "!", er

        if not data.get("text"):
            data["text"] = _get_latest_version(data.get("attribute-version", []))
...
def extract_data(base_data, node, depth=1, path=""):
    ...
    try:
        ...
        if data.get("tag") == "entity":
            ...
            definition = base_data["entity"][data["id"]].get("definition", "").lower()
            ## To make the logic more readable.
            # Break the masive if statement open.
            del_entry = False
            if "requirement" in definition:
                if number and (
                    number.startswith("VR.CM.")
                    or number.startswith("VE.CM.")
                    or number.startswith("R.CM.")
                    or number.startswith("VE.")
                    or number.startswith("CBF.V.")
                ):
...
def re_index_to_kat_id(data):
        ...
        for rel_type in relationship:
            ...
            for rel_id in relationship[rel_type]:
                ...
                if rel_num:
                    ...
                    if rel_type == "categorized by" and rel_num.startswith("Timescale"):
                        new_data[num]["timescale"] = rel_num
            if not new_data[num]["relationship"][rel_type]:
                del (new_data[num]["relationship"][rel_type])

    # Update timescale.
    for num in [n for n in new_data if new_data[n]["definition"] == "VerificationRequirement"]:
        timescale = None
    ...
    if data.get("export"):
        ...
        new_data["__Meta"]["processed"] = {"time": str(datetime.datetime.utcnow())}
    return new_data
