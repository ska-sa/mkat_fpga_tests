# Generated by bootstrap.sh from spead2/tools/send_asyncio.py. Do not edit.
...
def get_args():
    ...
    group.add_argument('--buffer', type=int, default=spead2.send.asyncio.UdpStream.DEFAULT_BUFFER_SIZE, help='Socket buffer size  [%(default)s]')
...
def run(item_group, stream, args):
    ...
    for is_end in rep:
        if len(tasks) >= 2:
            yield from(asyncio.wait([tasks.popleft()]))
        if is_end:
            task = asyncio.async(stream.async_send_heap(item_group.get_end()))
        else:
            for item in item_group.values():
                item.version += 1
            task = asyncio.async(stream.async_send_heap(item_group.get_heap()))
        tasks.append(task)
    while len(tasks) > 0:
        yield from(asyncio.wait([tasks.popleft()]))
...
def main():
    ...
    try:
        asyncio.get_event_loop().run_until_complete(run(item_group, stream, args))
