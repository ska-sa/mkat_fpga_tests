...
class CorrelatorFixture(object):
    ...
    def start_correlator(self, retries=5, loglevel='INFO'):
        ...
        katcp_port = subprocess.Popen("kcpcmd -s localhost:7147 array-list array{0}\
        ...
        while retries and not success:
            ...
            subprocess.check_call(['kcpcmd' '-s' 'localhost:{}'.format(array_port), 'instrument-activate', '{}'.format(mode)])

            subprocess.check_call(['kcpcmd', '-t', '30', '-s', 'localhost:{}'.format(port),
                'array-assign', 'array{}'.format(array_no), '{}'.format(multicast_ip)])
