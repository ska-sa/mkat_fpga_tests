...
def analyse_beam_data(bf_raw, skarab_or_roach = True, do_save = False, dsim_settings = [54832000, 0.08, 0.05], cbf_settings = [8191, 110], spectra_use = 'all', chans_to_use = 2**9, xlim = [20,21], dsim_factor = 1.0, ref_input_label='dummy', bandwidth=856e6, n_chans=4096):
    ...
    '''
    #%
    if do_save: plt.ioff()  # dont plot to screen
    ...
    print "choosing approximately %.5f seconds of data (NB, this is the extent of the data file)"%(len(nsamp_use)*t0_or[1])
    ...
    freq_chans_plt = range(0,len(dn_lim_cmplx0[:,0]))   # frequency channel numbers
...
if __name__ == "__main__":
    ...
    fname = ['bf_raw_1000.np.npy', 'bf_raw_0100.np.npy', 'bf_raw_0010.np.npy', 'bf_raw_0001.np.npy'] # place all beam data files in list to analyse
    path = './' # path to files
    ...
    analyse_beam_data(bf_raw, skarab_or_roach = True, do_save = True, dsim_settings = [13372910.15625, 0.675, 0.45], cbf_settings = [8191, 5], spectra_use = 'all', chans_to_use = 2**9, xlim = [50,52], dsim_factor = 1.0, ref_input_label='dummy')
